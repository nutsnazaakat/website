 ...
 *
 * Every request carries cookies, echoes the CSRF cookie on state-changing methods, unwraps the
 * `{ success, data }` envelope, and turns a failure into an `ApiRequestError` carrying the
 * backend's stable `code`. Nothing above this file should ever call `fetch` directly.
 *
 ...
 * branches on `code` alone therefore sees nothing for the one failure a customer is most likely to
 * hit by accident, so rate limiting must be detected as `status === 429`.
 */
 ...
 *
 * `GET /auth/me` breaks the same way: a returning visitor with an expired access cookie and 29 days
 * of refresh validity left would 401, hydrate as `null`, and be shown a signed-out site.
 */
 ...
 *
 * `refreshInFlight` is per tab, and that gap is not theoretical. Two tabs open on the site, both
 * idle past the access cookie's twenty minutes, both waking to a 401, each fire their own refresh.
 ...
 * open is ordinary behaviour, so it gets reported as "it randomly logs me out" — a description
 * nobody would connect to refresh-token rotation.
 *
...
interface RequestOptions {
  ...
  body?: unknown;
  /**
   * Extra request headers, merged over the two this client sets itself.
   *
   * Added for `Idempotency-Key` on `POST /checkout/orders`, which is the first header a caller has
   * ever needed to supply: `IdempotencyInterceptor` reads it off `request.headers` and no body field
   * could carry it. Kept as a general option rather than a `postWithKey` special case, and carried
   * through the refresh-and-retry below with the rest of `options` — a retry that dropped the key
   * would present the same request under no key at all and place a second order, which is precisely
...
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  // Caller headers last, so a caller can never silently lose the CSRF token it did not know about.
  ...
  if (response.status === 401 && !options.retry && !NEVER_REFRESH.has(path)) {
    ...
    // 401 — `GET /auth/me` legitimately refreshes on 401, and an anonymous visitor always 401s.
    // That is wasted traffic and log noise on every first paint, and it is measured against the
    // refresh route's 10/min/IP limit, so a visitor reloading a public page repeatedly could
    // rate-limit the endpoint their eventual sign-in depends on.
    //
    // A false negative is possible and cheap: a visitor who deletes only `nn_csrf` while keeping a
...
export const http = {
  ...
  post: <T>(path: string, body?: unknown, headers?: Record<string, string>) =>
    apiRequest<T>(path, { method: "POST", body, ...(headers ? { headers } : {}) }),
