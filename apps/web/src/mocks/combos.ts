/**
 * Brief §23 — the six named combos.
 *
 * A combo is a catalogue product in the `combos` category (the box you actually buy)
 * plus the list of retail packs that go inside it. Only slugs and pack sizes are stored:
 * every rupee figure on the combos page is derived from the catalogue at read time in
 * `catalogApi.listCombos`, so a price edit can never leave a stale savings claim behind.
 *
 * Each box holds 4 × 250g, which is why the box's own 1kg pack is the combo price.
 */
export interface ComboSeed {
  /** Slug of the catalogue product that is the box itself. */
  slug: string;
  occasion: string;
  blurb: string;
...
export const comboSeeds: ComboSeed[] = [
  {
  ...
  },
  {
    ...
    occasion: "Festive",
    blurb:
      "The Diwali and Eid standard — pistachios and medjool alongside the everyday two.",
    components: [
      { slug: "premium-california-almonds", size: "250g" },
      { slug: "w320-cashews", size: "250g" },
      { slug: "premium-pistachios", size: "250g" },
