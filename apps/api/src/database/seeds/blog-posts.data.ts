import { IMAGES } from './seed-context';

/**
 * All 8 posts from `frontend/src/mocks/posts.ts`, ported verbatim — bodies, excerpts and
 * publication dates included. Brief §28.
 *
 * The array literal below was lifted mechanically from that file rather than retyped, because
 * the bodies run to several thousand words and a hand-copy would drift. `img.*` became
 * `IMAGES.*` and nothing else changed.
 *
 * `readingMinutes` is deliberately absent: it is derived from the body at read time, so a
 * stored value could disagree with the text beside it.
 *
 * The bodies stay descriptive. Nothing here asserts a certification, a test result or a health
 * outcome — brief §25 forbids unsupported claims, and a blog post is exactly where one tends to
 * slip in unnoticed.
 */
